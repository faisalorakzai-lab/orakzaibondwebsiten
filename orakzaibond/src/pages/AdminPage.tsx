...
import {
  ShieldCheck, Loader2, CheckCircle2, ExternalLink, Bot, Save, Star,
...
import { useFounderRank, setFounderRank, FOUNDER_RANK_DEFAULT } from "@/hooks/useFounderRank";
...
type AdminTab = "dashboard" | "analytics" | "notifications" | "Lottery" | "players" | "treasury" | "vault" | "staking" | "lending" | "community" | "threat" | "defense" | "ai-settings" | "ambassador" | "legal-cms";
...
const NAV: { id: AdminTab; label: string; icon: JSX.Element; soon?: boolean; external?: string }[] = [
  ...
  { id: "ambassador",   label: "Ambassadors",   icon: <Star className="w-4 h-4" /> },
  { id: "legal-cms",    label: "Legal CMS",     icon: <Lock className="w-4 h-4" /> },
...
export default function AdminPage() {
  ...
  return (
    ...
    >
      ...
      {/* ── Top bar ──────────────────────────────────────────────────────────────
           Chairman directive 2026-04-30 (round 6) — definitive layout fix.
           
           Previous bug: on mobile (≤375 CSS px) the right-hand action group
           (POL pill, bell, sign-out, wallet pill) overflowed the 100vw row
           and got squeezed to a 1-character-wide flex column, which forced
           "OKBOND Command Center" + "OWNER ONLY" on the left to also
           collapse into vertical stacks of single letters. The screenshot
           Chairman shared made this unmistakable.
           
           Fix:
           • flex-wrap so children genuinely wrap when they exceed the row
           • min-w-0 on every flex child whose content is text so the text
             can shrink/ellipsize instead of overflowing
           • flex-shrink-0 on icon-only buttons so they keep their square
             touch target on every breakpoint
           • The two main groups (left identity, right actions) stack as
             two full-width rows on phones (flex-col on <sm) and revert to
             a single horizontal row from sm: upward */}
      <div className="relative z-20 border-b border-border/50 glass-dark px-3 sm:px-6 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
        <div className="flex items-center gap-3 min-w-0 flex-wrap">
          {isAdmin && (
            <button
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              className="md:hidden p-2 rounded-lg border border-border hover:border-primary/40 text-muted-foreground hover:text-primary transition-all flex-shrink-0"
            >
              <Menu className="w-5 h-5" />
            </button>
          )}
          <a href="/" className="flex items-center gap-2 text-muted-foreground hover:text-primary transition-colors text-sm flex-shrink-0">
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2 min-w-0 flex-wrap">
              <span
                className="text-foreground text-sm sm:text-base leading-tight whitespace-nowrap"
                style={{
              ...
              <span className="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase bg-primary/10 text-primary border border-primary/20 tracking-widest whitespace-nowrap flex-shrink-0">
                OWNER ONLY
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap min-w-0">
          ...
          {address ? (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-primary/30 bg-primary/10 whitespace-nowrap flex-shrink-0 max-w-full">
              <span className={`w-2 h-2 rounded-full flex-shrink-0 ${isAdmin ? "bg-green-400 animate-pulse" : "bg-red-400"}`} />
              <span className="text-primary font-mono text-xs truncate">{short(address)}</span>
            </div>
          ) : (
      ...
      <div
        className="relative z-20 px-3 sm:px-6 py-1.5 flex items-center justify-between text-[10px] font-mono uppercase tracking-[0.18em] flex-wrap gap-x-4 gap-y-1"
        style={{
            ...
            <AnimatePresence mode="wait">
              ...
              {tab === "dashboard" && (
                <motion.div key="dashboard" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-6">
                  ...
                  {/* ── Founder Identity: Crunchbase Rank editor (admin-only) ── */}
                  <FounderRankCard />
                </motion.div>
              )}
              ...
              {tab === "ambassador" && (
                <motion.div key="ambassador" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-6">
                  <div>
                    <h2 className="text-2xl font-extrabold text-foreground">Ambassador <span className="text-primary">Registrations</span></h2>
                    <p className="text-muted-foreground text-sm mt-0.5">Review all ambassador applications submitted via /ambassador. Data stored in localStorage of each user and (if configured) Supabase.</p>
                  </div>
                  <AmbassadorRegistrationsPanel />
                </motion.div>
              )}

              {/* ─── Legal CMS ────────────────────────────────────── */}
              {tab === "legal-cms" && (
                <motion.div key="legal-cms" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-6">
                  <div>
                    <h2 className="text-2xl font-extrabold text-foreground">Sovereign Transparency <span className="text-primary">Vault CMS</span></h2>
                    <p className="text-muted-foreground text-sm mt-0.5">Manage content displayed on the /legal page — corporate entity details, asset backing cards, and document vault links.</p>
                  </div>
                  <LegalCMSPanel />
                </motion.div>
              )}
...
// ──────────────────────────────────────────────────────────────────────────
// FounderRankCard — Chairman Orakzai's directive 2026-04-30:
//   "Give the Admin Wallet full control to change this rank via the Admin
//    Panel."
// Renders inside the Dashboard tab of the Power Console. Lets the connected
// admin wallet set the Crunchbase rank that shows on the Founder page and on
// the homepage Founder section. Persists in localStorage and broadcasts a
// "okbond:founder-rank-changed" CustomEvent so all open tabs / components
// re-render instantly without a refresh.
// ──────────────────────────────────────────────────────────────────────────
function FounderRankCard() {
  const liveRank = useFounderRank();
  const [draft, setDraft] = useState<string>(String(liveRank));
  const [saved, setSaved] = useState(false);

  useEffect(() => {
    setDraft(String(liveRank));
  }, [liveRank]);
  ...
  const dirty = String(liveRank) !== draft.trim();

  return (
    <div className="glass-gold rounded-3xl border border-primary/20 p-8 relative overflow-hidden">
      <div className="absolute top-0 right-0 p-8 opacity-10">
      ...
      <div className="relative z-10">
        ...
        <p className="text-muted-foreground text-sm mb-6 max-w-xl leading-relaxed">
          Controls the rank badge on the Founder page (hero + signature +
          social bar) and the Crunchbase pillar on the homepage Founder
          section. Default is <span className="text-primary font-semibold">#{FOUNDER_RANK_DEFAULT}</span>.
          Changes apply instantly across every open tab.
        </p>

        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center rounded-xl border border-primary/30 bg-background/50 overflow-hidden focus-within:border-primary/60 focus-within:shadow-[0_0_20px_rgba(234,179,8,0.25)] transition-all">
            <span className="px-3 text-primary font-bold text-lg select-none">#</span>
            <input
              type="number"
              min={1}
              inputMode="numeric"
              ...
              className="bg-transparent py-2.5 pr-4 w-32 text-2xl font-extrabold text-foreground font-mono outline-none"
              aria-label="Founder rank"
              data-testid="input-founder-rank"
            />
          </div>
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl font-extrabold text-sm transition-all flex items-center gap-2 ${
              dirty
                ? "bg-primary text-primary-foreground hover:bg-primary/90 shadow-[0_0_20px_rgba(234,179,8,0.35)]"
                : "bg-muted/30 text-muted-foreground cursor-not-allowed"
            }`}
            data-testid="button-save-founder-rank"
          >
            <Save className="w-4 h-4" />
            {saved ? "Saved" : "Save Rank"}
          </button>

          <button
            onClick={reset}
            className="px-4 py-2.5 rounded-xl border border-border text-xs font-bold text-muted-foreground hover:text-primary hover:border-primary/30 transition-all"
            data-testid="button-reset-founder-rank"
          >
            Reset to #{FOUNDER_RANK_DEFAULT}
          </button>

          {saved && (
            <span className="text-xs text-green-400 font-bold flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5" /> Live across the site.
            </span>
          )}
        </div>

        <div className="mt-5 p-3 rounded-xl bg-primary/5 border border-primary/15 text-xs text-muted-foreground">
          <Info className="w-3.5 h-3.5 text-primary inline mr-1.5 mb-0.5" />
          Persisted per-browser via localStorage. To sync across devices,
          back this with a Vercel KV write in <code className="text-primary/80">/api/founder-rank</code>.
        </div>
      </div>
    </div>
  );
...
// ── AmbassadorRegistrationsPanel ─────────────────────────────────────────────
function AmbassadorRegistrationsPanel() {
  const [registrations, setRegistrations] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    try {
      const stored = localStorage.getItem("okbond_ambassador_registrations");
  ...
  if (loading) {
    return <div className="flex items-center gap-2 text-muted-foreground"><Loader2 className="w-4 h-4 animate-spin" /> Loading registrations…</div>;
  }
  if (registrations.length === 0) {
    return (
      <div className="glass-dark border border-border rounded-2xl p-8 text-center text-muted-foreground">
        <Star className="w-8 h-8 mx-auto mb-3 opacity-30" />
        <p className="font-semibold">No ambassador registrations yet.</p>
        <p className="text-xs mt-1">Applications submitted via /ambassador will appear here.</p>
      </div>
    );
  }
  return (
    <div className="space-y-3">
      {registrations.map((r, i) => (
        <div key={i} className="glass-dark border border-border rounded-xl p-4 flex flex-wrap gap-4 items-start">
          <div className="flex-1 min-w-0 space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-bold text-foreground">{r.name || "—"}</span>
              <span className={`text-[10px] uppercase tracking-widest px-2 py-0.5 rounded-full font-bold ${
                r.tier === "Diamond" ? "bg-cyan-400/15 text-cyan-300 border border-cyan-400/30" :
                r.tier === "Gold"    ? "bg-primary/15 text-primary border border-primary/30" :
          ...
          <div className="text-right">
            <p className="text-[10px] text-muted-foreground font-mono">{r.timestamp ? new Date(r.timestamp).toLocaleDateString() : "—"}</p>
            <span className="text-[10px] text-green-400">Code of Conduct ✓</span>
          </div>
        </div>
      ))}
    </div>
...
// ── LegalCMSPanel ─────────────────────────────────────────────────────────────
function LegalCMSPanel() {
  const ASSETS_KEY = "okbond_legal_asset_notes";
  const [assetNotes, setAssetNotes] = useState<string>(() => {
    try { return localStorage.getItem(ASSETS_KEY) || ""; } catch { return ""; }
  });
  const [saved, setSaved] = useState(false);

  function saveNotes() {
    try { localStorage.setItem(ASSETS_KEY, assetNotes); } catch { /* */ }
    setSaved(true);
    ...
    <div className="space-y-5">
      <div className="glass-gold rounded-2xl border border-primary/20 p-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-foreground">Legal Page — Asset Backing Notes</h3>
            <p className="text-xs text-muted-foreground">These notes are persisted in localStorage and will be visible on the /legal page to admin sessions only.</p>
          </div>
        </div>
        <textarea
          rows={6}
          className="w-full rounded-xl border border-border bg-muted/10 px-4 py-3 text-sm text-foreground resize-y focus:outline-none focus:border-primary/50"
          placeholder="Add internal notes about asset documentation, pending verifications, or legal status updates…"
          value={assetNotes}
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={saveNotes}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-primary-foreground font-bold text-sm hover:bg-primary/90 transition-all shadow-[0_0_20px_rgba(234,179,8,0.3)]"
          >
      ...
      <div className="glass-dark border border-border rounded-2xl p-6 space-y-3">
